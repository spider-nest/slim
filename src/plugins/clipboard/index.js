import TinyEmitter from "$plugins/tinyEmitter";
import goodListen from "$plugins/goodListener";
...
export default class Clipboard extends TinyEmitter {
  ...
  resolveOptions(options = {}) {
    this.action = isFunction(options.action)
      ? options.action
      : this.defaultAction;
    this.target = isFunction(options.target)
      ? options.target
      : this.defaultTarget;
    this.text = isFunction(options.text) ? options.text : this.defaultText;
    this.container = isObject(options.container)
      ? options.container
      : document.body;
  }
   ...
   */
  listenClick(trigger) {
    this.listener = goodListen(trigger, "click", (e) => this.onClick(e));
  }
  ...
  static isSupported(action = ["copy", "cut"]) {
    const actions = isString(action) ? [action] : action;
